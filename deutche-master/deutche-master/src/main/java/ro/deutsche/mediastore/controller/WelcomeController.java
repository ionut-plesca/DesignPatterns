...
public class WelcomeController {
	...
	public String getCartStatement(Client client) {
		StringBuilder report = new StringBuilder("report for " + client.getName()+"\n");
		for(Rental rental: client.getRentals()) {
			report.append(rental.getProduct().getName()+", ");
			System.out.println(rental.getProduct().getName());
		}
		return report.substring(0, report.length()-2).toString();
	}
